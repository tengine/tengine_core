  ...
  describe :__evaluate__ do
    ...
    context "DSLのファイルを指定する場合" do
      ...
      it "イベントハンドラ定義を評価して、ドライバとハンドラを保持する" do
        ...
        @binder.__block_bindings__[[@handler1.filepath, @handler1.lineno].join('#') ].call
      end
    ...
    context "DSLのファイルを指定しない場合" do
      ...
      it "イベントハンドラ定義を評価して、ドライバとハンドラを保持する" do
        ...
        @binder.__block_bindings__[ [@handler2_1.filepath, @handler2_1.lineno].join('#') ].call

        @binder.should_receive(:puts).with("handler03_1")
        @binder.should_receive(:puts).with("handler03_2")
        @binder.__block_bindings__[ [@handler3_1.filepath, @handler3_1.lineno].join('#') ].call
        @binder.__block_bindings__[ [@handler3_2.filepath, @handler3_2.lineno].join('#') ].call
      end
    end
  end
